...
if __name__ == '__main__':
    ...
    for i in range(1,4):
        ...
        #plt.show()
    ##删除缺失值比率超过40%的变量
    df_1, na_del = del_na(df_1, list(df_1.columns), rate=0.4)
    len(na_del)
    ##删除行全为缺失值
...
##########################GBDT特征###################################
    data_train, data_test = train_test_split(df_1,test_size=0.2,random_state=0,stratify=df_1.target)
    ##区分离散变量与连续变量
    ...
    ##训练集类别变量WOE编码 用于xgboost特征生成
    var_disc_bin = list(df_disc_bin_train.columns)
    var_disc_bin.remove('target')
    df_train_disc_woe, dict_woe_disc_map, dict_iv_disc_values, var_woe_disc_name = var_encode.woe_encode(df_disc_bin_train, data_path,
                                                                                     var_disc_bin, df_disc_bin_train.target,
                                                                                     'dict_disc_woe_map', flag='train')
    ##测试集WOE编码
    ...
    ##测试集类别变量WOE编码 用于xgboost特征生成
    df_test_disc_woe, var_woe_disc_name = var_encode.woe_encode(df_disc_bin_test, data_path,
                                                       var_disc_bin,
                                                       df_disc_bin_train.target,
                                                       'dict_disc_woe_map', flag='test')
    ...
    ####取出xgb特征生成数据与xgb特征生成数据
    x_gbtrain = df_train_disc_woe[var_woe_disc_name].join(data_train[numerical_var])
    x_gbtest  = df_test_disc_woe[var_woe_disc_name].join(data_test[numerical_var])
    ...
    optimal_params = xgboost_bayesian_optimization(params_test, x_gbtrain, y_train,x_gbtest,y_test, init_points=5,
                                                   n_iter=8)

    print("贝叶斯调参最优参数: ", optimal_params)

    ##训练xgboost模型
    final_xgb_model = train_xgb(optimal_params, x_gbtrain, y_train,x_gbtest,y_test)
    auc_score = roc_auc_score(y_test, final_xgb_model.predict(xgb.DMatrix(x_gbtest)))
    print("贝叶斯调参模型AUC: ", auc_score)

    ###用apply方法得到树的映射结果
    train_new_feature = final_xgb_model.predict(xgb.DMatrix(x_gbtrain),pred_leaf=True)
    test_new_feature = final_xgb_model.predict(xgb.DMatrix(x_gbtest),pred_leaf=True)
...
#######################################特征筛选#####################################
    x_y_train = x_train_1.join(y_train)
    x_y_train.columns = x_y_train.columns.astype(str)
    # 利用toad库筛选特征
    quality_data=toad.quality(x_y_train,'target',iv_only=True)
    train_selected, dropped = toad.selection.select(x_y_train,target = 'target', iv = 0.06, corr = 0.6, return_drop=True)
    ...
    x_new = sf.fit_transform(x, x_y_train.target)
...
##########################################样本不均衡处理######################################
    ...
    df_1 = x_y_train[selected_cols].join(x_y_train["target"])
    ####尝试一点样本生成#####
    ##随机抽取一些样本
    ...
    x_y_test = x_test_1.join(y_test)
    x_y_test.columns = x_y_test.columns.astype(str)
    x_test = x_y_test[var_name]


    del_list = []
    for s in var_name:
        index_s = x_test[s].isnull()
        if sum(index_s) > 0:
            del_list.extend(list(x_test.index[index_s]))
    if len(del_list) > 0:
        list_1 = [x for x in list(x_test.index) if x not in del_list]
        x_test = x_test.loc[list_1]

        x_test = np.array(x_test)
        y_test = np.array(x_y_test.target.loc[list_1])
    else:
        x_test = np.array(x_test)
        y_test = np.array(x_y_test.target)
...
#####################################logistic模型#######################################
    ...
    lr_gsearch = GridSearchCV(
        estimator=LogisticRegression(random_state=0, fit_intercept=True, penalty='l2', solver='saga',max_iter=500),
